...
class Login extends React.Component {
  ...
  handleChange = (event) => {
    ...
    if (
      this.state.func === this.state.sfunc &&
      ...
      if (passVal.length >= 7) {
        ...
        this.setState({
          alertP: (
            <Alert message="Type at least 7 caracters" type="error" showIcon />
          ),
        });
      }
    }
    if (this.state.passwordRepeat !== "") {
      if (
        ...
        this.setState({
          alertP: (
            <Alert message="Passwords must match!" type="error" showIcon />
          ),
        });
      } else {
        this.setState({
          alertP: <Alert message="Passwords match!" type="success" showIcon />,
        });
      }
    }
  };
  comparePass = (event) => {
    ...
    if (passVal === this.state.password && passVal.length >= 7) {
      ...
      this.setState({
        alertP: <Alert message="Passwords must match!" type="error" showIcon />,
      });
    }
  };
  ...
  render() {
          ...
          <div>
            <DatePicker
              ...
              onChange={(date, dateString) =>
                this.datePickValue(date, dateString)
              }
            />
            <br />
          </div>
    ...
    var nButtRegist = (
      <div className={this.state.classE}>
        <Button type="link" onClick={method}>
          {this.state.nuser}
        </Button>
      </div>
    );
